...
you every five hours, an AI astrologer you can ask anything for free, astrologer
...
built by `src/lib/chart-brief.ts`: your name, gender, birth date, birth time and
birth place, the kundli `src/lib/kundli.ts` computes from them — moon sign,
nakshatra and pada, sidereal sun sign, estimated lagna, tithi and vara at birth
— and where the moon is *right now* against that chart, including the house it
is transiting from your natal moon. You never have to open the Kundli screen
first: if there is no chart yet, one is generated from the birth details on the
spot. Every answer is read from that one person's chart.

**What he will not do.** The brief states plainly which placements are computed
and which are not, and he is told never to name a graha, dasha or yoga he was
not given. He will not guarantee an outcome or a date, will not read death,
illness or pregnancy from a chart, and will not give medical, legal or financial
instructions. The chat says under every conversation that he is an AI.
